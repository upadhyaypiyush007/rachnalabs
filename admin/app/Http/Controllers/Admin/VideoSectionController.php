...
class VideoSectionController extends Controller
{
    ...
    public function index()
    {
        try{
            ...
            return view('admin.video_section.index', ['type'=>$type, 'category'=>$category]);
        } catch (Exception $e) {
    ...
    public function update(Request $request)
    {
        try{
            ...
            if($request->video_type ==1 ){
                ...
                $video = TVShow::select('*')->where('type_id',$request->type_id)->get();
            } else if ($request->video_type == 3) {
                $video = Language::select('*')->get();
            } else if ($request->video_type == 4) {
                $video = Category::select('*')->get();
            }
        ...
        if($request->is_home_screen == 1){
            ...
            for ($i=0; $i < count($data); $i++) { 
                ...
                if($data[$i]['video_type'] == 1){
                    ...
                    $video = Language::select('id','name')->whereIn('id', explode(",", $data[$i]['language_id']))->get();
                    $videos = array();
                    for ($j=0; $j < count($video); $j++) {
                        $videos[] = $video[$j]['name'];
                    }
                    ...
                    $video = Category::select('id','name')->whereIn('id', explode(",", $data[$i]['category_ids']))->get();
                    $videos = array();
                    for ($j=0; $j < count($video); $j++) {
                        $videos[] = $video[$j]['name'];
                    }
