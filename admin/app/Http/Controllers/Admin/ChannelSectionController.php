...
class ChannelSectionController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return view('admin.channel_section.index', ['type' => $type, 'category' => $category, 'channel' => $channel]);
        } catch (Exception $e) {
            ...
            if (Auth::guard('admin')->user()->type != 1) {
                ...
                if ($validator->fails()) {
                    ...
                    if ($request->video_type == '1') {
                        ...
                        $TVShowId = implode(',', $request->tv_show_id);
                        $data->video_id = "";
    ...
    public function update(Request $request)
    {
        try {
            ...
            if ($Channel_section->video_type == 1) {
                $video = Video::select('*')->where('type_id', $Channel_section->type_id)->where('channel_id', $Channel_section->channel_id)->get();
            } else if ($Channel_section->video_type == 2) {
                $video = TVShow::select('*')->where('type_id', $Channel_section->type_id)->where('channel_id', $Channel_section->channel_id)->get();
            }
